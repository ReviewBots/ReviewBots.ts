...
export class RBClient extends EventEmitter {
  ...
  public async getBotReviews(botID: Snowflake): Promise<ReviewInfo> {
    if (!botID)
      throw new Error(
        `[Review Bots API] Please provide a valid Bot ID to fetch reviews for!`
      );
    return this._request("GET", `rev/${botID}`);
  }

  public async getReviewInfo(
    botID: Snowflake,
    userID: Snowflake
  ): Promise<ReviewInfo> {
    if (!botID)
      throw new Error(
        `[Review Bots API] Please provide a valid Bot ID to fetch a review for`
      );
    if (!userID)
      throw new Error(
        `[Review Bots API] Please provide the User ID of the person who posted the review!`
      );
